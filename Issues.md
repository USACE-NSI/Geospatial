...
4. **P-08 (`DateFT`)** whenever `SpatialReader` is open anyway.

## 8. Commands

```bash
rm -rf tests/**/obj tests/**/bin Nsi.Geospatial*/obj Nsi.Geospatial*/bin && dotnet build Geospatial.slnx -c Release
dotnet test 2>&1 | tail -1     # BASELINE = 209 (207 pass, 2 skip: P-03, P-04) as of #28: 185 + ctor-order + ContainsPoint + empty-argument x4 + infinite-receiver x3 + Contains x13 + Contains-fact + Contains-vs-ContainsPoint x3. Preserving work holds it; a new test moves it, say which.
