...
public static class GeometryMath
{
  ...
  )
  {
    ...
    for (int i = 0; i < pts.Count; i++)
    {
      ...
      total +=
        LongDeltaRadians(lon1, lon2) * (Math.Sin(ToRadians(lat1)) + Math.Sin(ToRadians(lat2)));
    }
  ...
  )
  {
    ...
    if (deltaAT > delta12)
      return Math.Min(radiusMeters * delta13, SphericalDistance(p, b, radiusMeters));
